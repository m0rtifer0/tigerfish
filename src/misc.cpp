...
// Returns the full name of the current Tigerfish version.
//
// For local dev compiles we try to append the commit SHA and
// commit date from git. If that fails only the local compilation
// date is set and "nogit" is specified:
//      Tigerfish dev-YYYYMMDD-SHA
//      or
//      Tigerfish dev-YYYYMMDD-nogit
//
// For releases (non-dev builds) we only include the version number:
//      Tigerfish version
std::string engine_version_info() {
    std::stringstream ss;
    ss << "Tigerfish " << version << std::setfill('0');
...
std::string engine_info(bool to_uci) {
    // Tigerfish is a fork of Stockfish. Original Stockfish copyright belongs
    // to the Stockfish developers; Tigerfish modifications are additional work
    // by the Tigerfish developers layered on top.
    return engine_version_info() + (to_uci ? "\nid author " : " by ")
         + "the Tigerfish developers (Stockfish fork – see AUTHORS file)";
}
