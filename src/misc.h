...
struct std::hash<Tigerfish::FixedString<N>> {
    std::size_t operator()(const Tigerfish::FixedString<N>& fstr) const noexcept {
        return Tigerfish::hash_bytes(fstr.data(), fstr.size());
    }
};
