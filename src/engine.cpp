...
Engine::Engine(std::optional<std::string> path) :
    ...
    // Higher values → more attacking, initiative-seeking play.
    options.add("TigerAggression", Option(50, 0, 100));

    // 0-100.  Reduces LMR for checking / forcing moves.
    // Higher values → attack / check lines explored more deeply.
    options.add("TigerRisk", Option(50, 0, 100));

    // 0-100.  Near-equality optimism bonus to prefer sharp over sterile lines.
    // Higher values → stronger anti-draw / complicating tendency.
    options.add("TigerAntiDraw", Option(50, 0, 100));
